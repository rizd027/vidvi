...
async function handleTwitter(url) {
  ...
  const scrapers = [
    ...
    async () => {
      const res = await fetch('https://twitsave.com/info', {
        method: 'GET',
        headers: {
          'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/120.0.0.0 Safari/537.36'
        },
...
app.get('/api/download/:platform', async (req, res) => {
  ...
  try {
    ...
    if (db) {
      await db.run(
        `INSERT INTO downloads (platform, url, title, thumbnail, duration, download_url, media_type, size)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [platform, url, dbTitle, dbThumbnail, dbDuration, dbDownloadUrl, mediaType, dbSize]
      );
    }
